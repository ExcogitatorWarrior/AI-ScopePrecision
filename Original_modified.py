...
#            input_image = add_salt_noise_single(input_image, 0.105 * counter) # this code used for denoise mode. 0.105 is main parameter
#            input_image, compare_image = add_salt_noise(input_image, compare_image, 0.105 * counter) # this code used for noised mode. 0.105 is main parameter
            ...
            if counter == 0:
                ...
                with torch.no_grad():
                    scope = extract_leftmost_side(decoded_image.detach()) # extracting scope.
            elif counter < 9:
                with torch.no_grad():
                    input_image = apply_leftmost_side(input_image, scope) # applying scope to image.
                decoded_image = model(input_image)
                with torch.no_grad():
                    scope = extract_leftmost_side(decoded_image.detach()) # extracting scope.
            if counter < 9:
                # Training model to utilize scope and gain precision by modification of loss.
                with torch.no_grad():
                    compare_image = modify_precision_leftmost_pixels(compare_image, image)
...
#    model.load_state_dict(torch.load('model_orig_mod_weights_noised.pth', map_location=device))
    ...
    torch.save(model.state_dict(), 'model_orig_mod_weights_deadman0.09.pth')

    plt.figure(figsize=(10, 6))
    plt.plot(losses)
    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    plt.title('Orig deadman 0.09 Loss Over Time')
    plt.show()