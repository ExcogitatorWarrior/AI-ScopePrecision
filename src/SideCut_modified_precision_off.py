...
#            input_image, compare_image = add_salt_noise(input_image, compare_image, 0.105 * counter) # This code used for noised mode. 0.105 is main parameter
            input_image = add_salt_noise_single(input_image, 0.105 * 9) # This code used for deadman mode. 0.105 is main parameter
            input_image, compare_image  = input_image.to(device), compare_image.to(device)
            if counter == 0:
                decoded_image = model(input_image)
            elif counter < 9:
                with torch.no_grad():
                    input_image = apply_leftmost_side_cutting(input_image) # Replace 28 leftmost pixels with 0.0 to feed.
                decoded_image = model(input_image)
            if counter < 9:
                with torch.no_grad():
                    compare_image = apply_leftmost_side_cutting(compare_image) # Replace 28 leftmost pixels with 0.0 to train.
...
#    model.load_state_dict(torch.load('model_orig_mod_weights_noised.pth', map_location=device))
    ...
    torch.save(model.state_dict(), 'model_cutside_mod_weights_deadman0.105.pth')

    plt.figure(figsize=(10, 6))
    plt.plot(losses)
    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    plt.title('Cut Side deadman 0.105 Loss Over Time')
    plt.show()